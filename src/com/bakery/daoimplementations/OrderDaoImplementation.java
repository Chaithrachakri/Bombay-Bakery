...
public class OrderDaoImplementation implements OrderDao {
	...
	public void insertOrder(String item,String quantity,String userName,String OID) throws ClassNotFoundException, SQLException
	{
		
		 Connection con = DBUtility.getConnection();
			PreparedStatement psmt = con.prepareStatement("Insert into orderplace(orderId,itemSelected,Quantity,UserName) values(?,?,?,?)");
	...
	public ArrayList placeOrder(String request,String userName,String OID) throws ClassNotFoundException,
		 ...
		 for(String item:itemList)
		 {
			 if(item.isEmpty())
			  ...
			  if(d!=null)
			  {
				  insertOrder(d.getitemName(),quantity,userName,OID);
				  itm.add(d);
			  }
		 }
		return itm;
	...
	{
		Connection con = DBUtility.getConnection();
	     PreparedStatement psmt = con.prepareStatement("select ItemPrice from Items where ItemName = ?");
	     psmt.setString(1, itemName);
	     ResultSet rs = psmt.executeQuery();
	     String price="0.00";
	     if(rs.next())
	...
	public ArrayList viewPendingOrders() throws ClassNotFoundException,
			SQLException {
		 Connection con = DBUtility.getConnection();
	     PreparedStatement psmt = con.prepareStatement("select * from orderPlace where status='PENDING'");
	        ResultSet rs = psmt.executeQuery();
			 ArrayList itm=new ArrayList();
	        while(rs.next()){
	            Order d=new Order();
